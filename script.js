  ...
  return (
    <div className="border rounded-lg p-4 flex flex-col shadow-sm hover:shadow-lg transition-shadow bg-white">
      ...
      <div className="flex justify-between">
        ...
        <button onClick={()=>onFav(p.id)}>{fav?'❤️':'🤍'}</button>
      </div>
      <div className="text-xs text-gray-600">{p.category}</div>
      <div className="mt-1 text-sm">{p.desc}</div>
      <div className="mt-1 text-yellow-600 text-sm">⭐ {p.rating.toFixed(1)}</div>
      ...
      <button onClick={()=>onReview(p)} className="text-xs mt-1 underline">รีวิว</button>
    </div>
  );
}
        ...
        <div className="p-4">
            <div className="relative h-64 bg-cover bg-center rounded-lg shadow-lg" style={{backgroundImage: "url('https://images.unsplash.com/photo-1542826189-9a706b8720ca?q=80&w=1974&auto=format&fit=crop')"}}>
                <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center rounded-lg">
                    <h2 className="text-white text-4xl font-bold drop-shadow-lg text-center">ยินดีต้อนรับสู่ Bakery Moon Sweet</h2>
                </div>
            </div>

            <div className="mt-8">
                <h3 className="font-bold text-2xl mb-4 text-center text-amber-800">สินค้าแนะนำ</h3>
            ...
            <div className="mt-8 bg-amber-200 p-6 rounded-lg shadow-md text-center">
                <h3 className="font-bold text-2xl text-amber-800">โปรโมชั่นพิเศษ!</h3>
                <p className="mt-2 text-amber-700 text-lg">รับส่วนลด 10% เมื่อสั่งซื้อครบ 500 บาท</p>
                <p className="font-semibold text-amber-900 mt-1">ใช้โค้ด: SWEET10</p>
            </div>
            
            <div className="mt-8 text-center text-gray-700">
                <h3 className="font-bold text-2xl text-amber-800">เกี่ยวกับเรา</h3>
                <p className="mt-2 text-lg">
                    เราอบขนมด้วยความรักและความใส่ใจทุกขั้นตอน
                    เพื่อมอบความสุขให้กับทุกคนผ่านรสชาติที่หอมหวานและนุ่มละมุน
                    มาร่วมแบ่งปันช่วงเวลาดีๆ กับเบเกอรี่ของเราสิครับ/ค่ะ!
                </p>
            </div>
        </div>
    );
...
function Cart({items,onQty,onRemove,onCheckout,coupon,onCoupon,onClose}){
  ...
  return (
    <div className="fixed top-20 right-4 w-80 bg-white border p-4 rounded shadow z-50">
      ...
      <input className="border p-1 w-full mb-2" placeholder="ใส่คูปอง" value={coupon} onChange={e=>onCoupon(e.target.value)}/>
  ...
  return (
    <div className="p-4">
      ...
      <ul className="list-disc ml-5">
        {favs.length?favs.map(f=><li key={f}>{f}</li>):<li>ไม่มีรายการโปรด</li>}
      </ul>
...
function App(){
  ...
  if(!user){
    const name = prompt("ใส่ชื่อผู้ใช้สำหรับจำลองการล็อกอิน");
